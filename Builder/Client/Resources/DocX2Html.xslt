...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
				...
				exclude-result-prefixes="w a o v WX aml w10 mml m pic wp r"
				version="1.0">
	...
	<xsl:template match="w:r//w:t[../w:rPr/WX:sym]">
		...
		<span>
			<!-- 20131205 Koert van Kleef: disabled fonts -->
<!--			<xsl:attribute name="style">
	...
	<!--	mh: added to add the images in the text. src have to refer to the relationshipId.
				the html converter will replace the relationshipId with the actual image path.
				the height and the width as defined in word is added and divided by 9525 which converts the value to pixels.-->
	<xsl:template match="w:drawing">
		<img>
			<xsl:attribute name="src">
				<xsl:value-of select="wp:inline/a:graphic/a:graphicData/pic:pic/pic:blipFill/a:blip/@r:embed"/>
			</xsl:attribute>
	...
	<xsl:template match="w:object">
		<img>
			<xsl:attribute name="src">
				<xsl:value-of select="v:shape/v:imagedata/@r:id"/>
			</xsl:attribute>
			<xsl:attribute name="width">
				<xsl:value-of select="(substring-before(substring-after(v:shape/@style,'width:'),'pt') * 96) div 72"/>
			</xsl:attribute>
			<xsl:attribute name="height">
				<xsl:value-of select="(substring-before(substring-after(v:shape/@style,'height:'),'pt') * 96) div 72"/>
			</xsl:attribute>
		</img>
		<br />
	</xsl:template>
					...
					<xsl:when test="./@w:pos">
						<xsl:choose>
							<xsl:when test="./@w:pos and ./@w:val = 'left'">
								margin-left:<xsl:value-of select="@w:pos div 20"/>pt;
							</xsl:when>
							<xsl:when test="./@w:pos and ./@w:val = 'right'">
								margin-right:<xsl:value-of select="@w:pos div 20"/>pt;
							</xsl:when>
						</xsl:choose>
					</xsl:when>
					<xsl:otherwise>
	...
	<xsl:template name="ApplyRPr.once">
		...
		<xsl:if test="$b.complexScript = $on">
			...
			<xsl:choose>
				...
				<!-- 4apr2014 Koert van Kleef Omit font-weight:normal because this is default and would not be in alignment with how Builder treats text -->
		...
		<!-- 29 nov 2013 Koert van Kleef: removed because Questify Builder already assumes ltr and rtl does not exist -->
		...
		<!--	mh: removed because some text was gray while it shouldn't -->
		<!--	<xsl:if test="substring($prsR,$iEmbossImprint,1) = $on">color:gray;</xsl:if> -->

		<!-- WouterB: removed because some text was underlined or stiken-through while it shouldn't -->
		<!-- Note that when the text should be underlined or striken-through, that still works despite this code being commented out.-->
		...
		<!-- 4apr2014 Koert van Kleef Removed possibly not a reliable way to determine super and subscript when the textrun has no other attributes -->
	...
	<xsl:template match="w:b" mode="rpr">
		<xsl:choose>
			...
			<xsl:when test="not(@w:val = 'off' or @w:val = '0')">font-weight:bold;</xsl:when>
		</xsl:choose>
	</xsl:template>

	<!-- 12-08-2014 WouterB: Added to make text tagged as "Strong" bold. -->
	<xsl:template match="w:rStyle" mode="rpr">
		<xsl:choose>
			<xsl:when test="(@w:val = 'Strong')">font-weight:bold;</xsl:when>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template match="w:i" mode="rpr">
		<xsl:choose>
			...
			<xsl:when test="not(@w:val = 'off' or @w:val = '0')">font-style:italic;</xsl:when>
		</xsl:choose>
	</xsl:template>
	<!--	mh: added to convert underlined text -->
	<xsl:template match="w:u" mode="rpr">
		<xsl:choose>
			<!--<xsl:when test="@w:val = 'off'">text-decoration:normal;</xsl:when>
			<xsl:otherwise>text-decoration:underline;</xsl:otherwise>-->
			<xsl:when test="not(@w:val = 'off' or @w:val = '0')">text-decoration:underline;</xsl:when>
		</xsl:choose>
	</xsl:template>
	<!-- 4apr2014 Koert van Kleef Added to (more reliably) convert superscript and subscript-->
	<xsl:template match="w:vertAlign" mode="rpr">
		<xsl:choose>
			<xsl:when test="@w:val = 'superscript'">vertical-align:super;</xsl:when>
			<xsl:when test="@w:val = 'subscript'">vertical-align:sub;</xsl:when>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template name="DisplayR">
		...
		<xsl:variable name="styleMod">
			...
			<xsl:if test="$isBullets=$on or ancestor::w:rt">
				...
				<xsl:text>font-style:normal;text-decoration:none;</xsl:text>
			</xsl:if>
		</xsl:variable>
		<xsl:choose>
			...
			<xsl:otherwise>
				<span>
					...
					<!-- 1 dec 2013 Koert van Kleef: removed because Questify Builder does not use the "Normal-P" or "Normal-H" class -->
					<!--<xsl:if test="not($rStyleId='')">
						<xsl:attribute name="class">
					...
					<!-- 4 apr 2014 Koert van KleefAll below disabled because Builder uses tags sup and sub instead of spans with style, so these size adjustments then cause a conflict -->
					<!--<xsl:choose>
...
-->
	...
	<xsl:template name="ColumnAddition">
		<!-- This is template is added by Parwati to handle columns -->
		...
		<xsl:if test="not($nInd='' and $pr.listInd='')">
			...
			<xsl:choose>
				...
				<xsl:when test="not($nInd.left = '')">
					<!-- Modified by Parwati to handle negative indentation -->
					<xsl:value-of select="$marginSide.before"/>
					<xsl:choose>
	...
	<xsl:template match="w:p">
		...
		<xsl:if test="not(w:pPr/w:pStyle/@w:val='z-TopofForm') and not(w:pPr/w:pStyle/@w:val='z-BottomofForm')">
			...
			<xsl:if test="./w:r/w:lastRenderedPageBreak/@ColumnEnd">
				<!-- Added by Parwati to handle columns -->
				<xsl:variable name="columnNo">
...
-->
			...
			<body>
				<xsl:if test="w:background/@w:color">
					<xsl:variable name="color">
						<!-- Added by Parwati to handle Background Page Color-->
						<xsl:text>#</xsl:text>
					</xsl:variable>
				...
				<!-- End of change by Parwati -->
				<xsl:if test="//w:body//w:sdt//w:sdtContent and //w:body/w:p/w:r/w:lastRenderedPageBreak">
					<!-- For coverpage Added by Parwati-->
	...
	<xsl:template match="/">
		...
		>
				type="text/xsl" href="<xsl:value-of select="$pmathml"/>"
  ...
  -->
	...
	<xsl:template match="m:oMath">
		<xsl:variable name="varAlign">
			<xsl:value-of select="preceding-sibling::m:oMathParaPr/m:jc/@m:val"/>
		</xsl:variable>
