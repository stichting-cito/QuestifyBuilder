...
Namespace QTI.Facade.Factories.QTI30
    ...
    Public Class ResponseTypeFactory
        ...
        Private ReadOnly _owner As CombinedScoringConverter
        Private ReadOnly _responseIdentifierAttributeList As XmlNodeList
        Private ReadOnly _finding As KeyFinding
        Private ReadOnly _controlType As QTI30CombinedScoringHelper.EnumControlType = QTI30CombinedScoringHelper.EnumControlType.Unknown
        ...
        Public Sub New(scoringParameters As HashSet(Of ScoringParameter), finding As KeyFinding, responseIdentifierAttributeList As XmlNodeList, owner As CombinedScoringConverter)
            If scoringParameters IsNot Nothing AndAlso scoringParameters.Count > 0 AndAlso _factIdsPerScoringParameter.Count = 0 Then
                _factIdsPerScoringParameter = QTI30CombinedScoringHelper.GetFactIdsFromScoringParameters(scoringParameters)
                _responseIdentifierAttributeList = responseIdentifierAttributeList
                _finding = finding
                _owner = owner
                _responseIndexPerIdentifier = QTI30CombinedScoringHelper.DetermineResponseIndexPerIdentifier(_responseIdentifierAttributeList)
                FillResponseIndex()
        ...
        Public Sub New(solution As Solution, finding As KeyFinding, responseIdentifierAttributeList As XmlNodeList, owner As CombinedScoringConverter)
            _controlType = QTI30CombinedScoringHelper.DetermineControlType(solution, responseIdentifierAttributeList, finding)
            _responseIdentifierAttributeList = responseIdentifierAttributeList
            _finding = finding
            _owner = owner
            _responseIndexPerIdentifier = QTI30CombinedScoringHelper.DetermineResponseIndexPerIdentifier(_responseIdentifierAttributeList)
        ...
        Public Function CreateResponseDeclarationTypeByFact(ByVal fact As KeyFact) As IResponseDeclaration
            ...
            If fact.Id IsNot Nothing AndAlso Not String.IsNullOrEmpty(fact.Id) Then
                ...
                ElseIf Not _controlType = QTI30CombinedScoringHelper.EnumControlType.Unknown AndAlso _responseIndexPerIdentifier.ContainsKey(factId) Then
                    Return GetResponseDeclarationTypeByControlType(fact)
                End If
            End If
            If Not _controlType = QTI30CombinedScoringHelper.EnumControlType.Unknown AndAlso fact.Values.Count > 0 AndAlso _responseIndexPerIdentifier.ContainsKey(fact.Values(0).Domain) Then
                Return GetResponseDeclarationTypeByControlType(fact)
            Else
        ...
        Private Function GetResponseDeclarationTypeByControlType(fact As KeyFact) As IResponseDeclaration
            ...
            Select Case _controlType
                Case QTI30CombinedScoringHelper.EnumControlType.MultipleChoice
                    responseDeclarationType = New ResponseDeclarationMultipleResponse()
                Case QTI30CombinedScoringHelper.EnumControlType.Choice
                    responseDeclarationType = New ResponseDeclarationChoice()
                Case QTI30CombinedScoringHelper.EnumControlType.Input
                    Dim responseIdentifierAttribute As XmlNode = GetResponseIdentifierForValue(fact)
                    Dim factGapType As QTI30CombinedScoringHelper.EnumGapType = QTI30CombinedScoringHelper.GetGapTypeByKeyValue(responseIdentifierAttribute, CType(fact.Values(0), KeyValue))
                    responseDeclarationType = New ResponseDeclarationInput(factGapType)
                Case QTI30CombinedScoringHelper.EnumControlType.Gapmatch
                    responseDeclarationType = New ResponseDeclarationGapMatch()
            End Select
            Return responseDeclarationType
        End Function
        ...
        Private Function GetConceptResponseProcessingTypeByScoringParameter(scoringPrm As ScoringParameter, responseIndex As Integer, Optional responseSubIndex As Integer = 0) As IResponseProcessing
            ...
            Select Case scoringPrm.GetType
                Case GetType(DateScoringParameter),
                    ...
                    If scoringPrm.SupportCasScoring AndAlso QTI30CombinedScoringHelper.IsDependencyGap(scoringPrm, _finding, _factIdsPerScoringParameter) Then
                        ret = New ConceptResponseProcessingInputFormula(responseIndex, QTI30CombinedScoringHelper.FormulaItemType.EvaluateDependency, _owner.GetResponseProcessingCustomOperators, GetDependentVariablesForFormulaGap(), responseSubIndex)
                ...
                Case GetType(MathScoringParameter),
                    ...
                    Dim formulaItemType As QTI30CombinedScoringHelper.FormulaItemType = QTI30CombinedScoringHelper.GetFormulaItemType(_finding, scoringPrm)
                    ...
                    Else
                        ret = New ConceptResponseProcessingInputFormula(responseIndex, formulaItemType, _owner.GetResponseProcessingCustomOperators, QTI30CombinedScoringHelper.IsMultiLineFormulaGap(CType(_responseIdentifierAttributeList(GetResponseIndexByIdentifier(GetIdFromScoringParameter(scoringPrm)) - 1), XmlAttribute)), responseSubIndex)
                    End If
                Case GetType(GapMatchScoringParameter), GetType(GapMatchRichTextScoringParameter)
        ...
        Private Function GetResponseProcessingTypeByScoringParameter(scoringPrm As ScoringParameter, responseIndex As Integer, Optional responseSubIndex As Integer = 0) As IResponseProcessing
            ...
            Select Case scoringPrm.GetType
                Case GetType(CurrencyScoringParameter),
                    ...
                    Dim gapType = QTI30CombinedScoringHelper.GetGapTypeByScoringParameter(scoringPrm)
                    If scoringPrm.SupportCasScoring AndAlso QTI30CombinedScoringHelper.IsDependencyGap(scoringPrm, _finding, _factIdsPerScoringParameter) Then
                        responseProcessing = New ResponseProcessingInputFormula(responseIndex, QTI30CombinedScoringHelper.FormulaItemType.EvaluateDependency, _owner.GetResponseProcessingCustomOperators, GetDependentVariablesForFormulaGap(), responseSubIndex)
                    ElseIf (gapType = QTI30CombinedScoringHelper.EnumGapType.DecimalGap OrElse gapType = QTI30CombinedScoringHelper.EnumGapType.CurrencyGap) AndAlso _responseIndexPerIdentifier.ContainsKey(GetIdFromScoringParameter(scoringPrm)) Then
                        responseProcessing = New ResponseProcessingInput(responseIndex, gapType, QTI30CombinedScoringHelper.DetermineDecimalSeparatorForGap(CType(_responseIdentifierAttributeList(GetResponseIndexByIdentifier(GetIdFromScoringParameter(scoringPrm)) - 1), XmlAttribute)), _owner.GetResponseProcessingCustomOperators, responseSubIndex)
                    ElseIf TypeOf scoringPrm Is GeogebraScoringParameter Then
                ...
                Case GetType(MathScoringParameter),
                    ...
                    Else
                        responseProcessing = New ResponseProcessingInputFormula(responseIndex, formulaItemType, _owner.GetResponseProcessingCustomOperators, QTI30CombinedScoringHelper.IsMultiLineFormulaGap(CType(_responseIdentifierAttributeList(GetResponseIndexByIdentifier(GetIdFromScoringParameter(scoringPrm)) - 1), XmlAttribute)), responseSubIndex)
                    End If
        ...
        Private Function GetResponseProcessingTypeByControlType(fact As KeyFact, responseId As Integer, scoringParameter As ScoringParameter) As IResponseProcessing
            ...
            Select Case _controlType
                ...
                Case QTI30CombinedScoringHelper.EnumControlType.Input
                    Dim responseIdentifierAttribute As XmlNode = GetResponseIdentifierForValue(fact)
                    Dim factGapType As QTI30CombinedScoringHelper.EnumGapType = QTI30CombinedScoringHelper.GetGapTypeByKeyValue(responseIdentifierAttribute, CType(fact.Values(0), KeyValue))
                    If factGapType = QTI30CombinedScoringHelper.EnumGapType.FormulaGap Then
                        Dim formulaItemType As QTI30CombinedScoringHelper.FormulaItemType = QTI30CombinedScoringHelper.GetFormulaItemTypeByFinding(_finding)
                        Debug.Assert(formulaItemType <> QTI30CombinedScoringHelper.FormulaItemType.EvaluateDependency, "Formula-item without scoringparameters, has formulaItemType 'EvaluateDependency'. Should not occur !")
                        ret = New ResponseProcessingInputFormula(responseId, formulaItemType, CType(_owner.GetResponseProcessingCustomOperators, ResponseProcessingCustomOperators), QTI30CombinedScoringHelper.IsMultiLineFormulaGap(CType(responseIdentifierAttribute, XmlAttribute)))
                    ElseIf factGapType = QTI30CombinedScoringHelper.EnumGapType.DecimalGap OrElse factGapType = QTI30CombinedScoringHelper.EnumGapType.CurrencyGap Then
                        ret = New ResponseProcessingInput(responseId, factGapType, QTI30CombinedScoringHelper.DetermineDecimalSeparatorForGap(CType(responseIdentifierAttribute, XmlAttribute)), CType(_owner.GetResponseProcessingCustomOperators, ResponseProcessingCustomOperators))
                    Else
        ...
        Private Function GetOrderIndexForScoringParameter(scoringPrm As ScoringParameter, domainOrderRetrievalType As QTI30CombinedScoringHelper.DomainOrderRetrievalType) As Dictionary(Of String, Integer)
            Return QTI30CombinedScoringHelper.GetDomainOrderFromScoringParameter(scoringPrm, domainOrderRetrievalType)
        End Function
                ...
                For Each scoringPrm As ScoringParameter In GetScoringParamFromFactIdsPerScoringParameterById(kvp.Key)
                    ...
                    If ciScoringPrmIndex > 0 Then
                        If TypeOf scoringPrm Is DecimalScoringParameter Then
                            Dim orderIndexPerScoringPrm As Dictionary(Of String, Integer) = GetOrderIndexForScoringParameter(scoringPrm, QTI30CombinedScoringHelper.DomainOrderRetrievalType.FactId)
                    ...
                    If scoringPrm.Value.Count > 1 AndAlso ((TypeOf scoringPrm Is MultiChoiceScoringParameter AndAlso Not scoringPrm.IsSingleValue) OrElse ((TypeOf scoringPrm Is ChoiceScoringParameter AndAlso Not TypeOf scoringPrm Is MultiChoiceScoringParameter) OrElse TypeOf scoringPrm Is GapMatchScoringParameter OrElse TypeOf scoringPrm Is GraphGapMatchScoringParameter OrElse TypeOf scoringPrm Is MatrixScoringParameter) AndAlso Not scoringPrm.IsSingleChoice) Then
                        Dim orderIndexPerScoringPrm As Dictionary(Of String, Integer) = GetOrderIndexForScoringParameter(scoringPrm, QTI30CombinedScoringHelper.DomainOrderRetrievalType.All)
                        For Each oiKvp As KeyValuePair(Of String, Integer) In orderIndexPerScoringPrm.OrderBy(Function(oiKvp2) oiKvp2.Value)
                            If Not _responseIndexPerIdentifier.ContainsKey(oiKvp.Key) Then _responseIndexPerIdentifier.Add(oiKvp.Key, CDbl(kvp.Value + (ciScoringPrmIndex / 100) + (oiKvp.Value / 10000)))
                        Next
                    ...
                    If TypeOf scoringPrm Is ChoiceScoringParameter OrElse TypeOf scoringPrm Is GapMatchScoringParameter OrElse TypeOf scoringPrm Is GraphGapMatchScoringParameter OrElse TypeOf scoringPrm Is MatrixScoringParameter Then
                        If ((TypeOf scoringPrm Is MultiChoiceScoringParameter AndAlso Not scoringPrm.IsSingleValue) OrElse Not TypeOf scoringPrm Is MultiChoiceScoringParameter) AndAlso Not scoringPrm.IsSingleChoice AndAlso scoringPrm.Value.Count > 1 Then
                            ...
                            If TypeOf scoringPrm Is GapMatchScoringParameter Then
                                orderIndexPerScoringPrm = GetOrderIndexForScoringParameter(scoringPrm, QTI30CombinedScoringHelper.DomainOrderRetrievalType.CollectionId)
                            ElseIf TypeOf scoringPrm Is GraphGapMatchScoringParameter Then
                                orderIndexPerScoringPrm = GetOrderIndexForScoringParameter(scoringPrm, QTI30CombinedScoringHelper.DomainOrderRetrievalType.FactId)
                            Else
                                orderIndexPerScoringPrm = GetOrderIndexForScoringParameter(scoringPrm, QTI30CombinedScoringHelper.DomainOrderRetrievalType.Domain)
                            End If
                            For Each oiKvp As KeyValuePair(Of String, Integer) In orderIndexPerScoringPrm.OrderBy(Function(oiKvp2) oiKvp2.Value)
        ...
        Private Function GetIndexForCustomInteractionScoringParameters(controllerId As String) As Integer
            If controllerId.StartsWith(ciControllerId) Then
                ...
                ElseIf controllerId.StartsWith(String.Concat(ciControllerId, "_"), StringComparison.OrdinalIgnoreCase) AndAlso controllerId.Substring(String.Concat(ciControllerId, "_").Length).IndexOf("_") > 0 Then
                    ...
                    If Not String.IsNullOrEmpty(controllerGuid) AndAlso QTI30CombinedScoringHelper.CheckIdentifierIsGuid(controllerGuid) Then
            ...
            _factIdsPerScoringParameter.Values.ToList.ForEach(Sub(fid)
                                                                  ...
                                                                  If Not String.IsNullOrEmpty(id) Then
                                                                      If id.Equals(identifier, StringComparison.OrdinalIgnoreCase) Then
                                                                          scoringPrms.Add(fid)
                                                                      ElseIf QTI30CombinedScoringHelper.GetGuidPartOfIdentifier(id).Equals(identifier, StringComparison.OrdinalIgnoreCase) Then
                                                                          scoringPrms.Add(fid)
                                                                      End If
                                                                  End If
                                                              End Sub)
