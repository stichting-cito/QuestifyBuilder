...
Namespace QTI.ChainHandlers.Processing.QTI22
    ...
    Public Class QTI22_SaveTestAndManifestHandler
        ...
        Protected ResourceTypes As ConcurrentDictionary(Of QTI22PackageCreator.QTIManifestResourceType, String)
        Public TimeStamper As ITimeStamp



        Public Sub New(testPackage As TestPackage, testPackageGuid As String, packageCreator As QTI22PackageCreator)
        ...
        Protected Overridable Sub BeforeManifestSave(requestData As QTI22PublicationRequest)
        End Sub
        ...
        Protected Overridable Sub SaveManifest(resources As ConcurrentDictionary(Of String, Dictionary(Of ResourceType, List(Of String))), manifest As ManifestType, ByRef filesPerEntity As ConcurrentDictionary(Of String, List(Of String)), css As ConcurrentBag(Of String))
            If Not css.Count = 0 Then
                Dim cssContent = String.Join(vbNewLine, css.ToArray)
                PackageCreator.SaveGeneratedCss(resources, cssContent, FolderDirectory(PackageCreatorConstants.FileDirectoryType.css), ResourceTypes(QTI22PackageCreator.QTIManifestResourceType.webcontent))
            End If
            ...
            GetNamespaceHelper().UpdateNameSpaces(manifestDocument, False)
            ...
            Validate(New String() {fileName}.ToList)
        End Sub
        ...
        Private Sub AddTestResourcesToManifest(requestData As QTI22PublicationRequest, testDocumentSet As TestDocumentSet)
            Dim testname = testDocumentSet.Test.Identifier.Replace(Chr(32), "_"c)
            Dim resourceType As ResourceType = QTI22PackageCreator.GetResourceType(requestData.Resources, ChainHandlerHelper.GetIdentifierFromResourceId(testname, PackageCreatorConstants.TypeOfResource.test))
            AddPropertiesToResourceType(requestData, testDocumentSet, resourceType, testname)
            Dim files = GetTestFiles(testname)
            QTI22PackageCreator.AddResourceToManifest(requestData.Resources, resourceType, files.ToArray)
        End Sub

        Protected Overridable Sub AddPropertiesToResourceType(requestData As QTI22PublicationRequest, testDocumentSet As TestDocumentSet, ByRef resourceType As ResourceType, testName As String)
            Dim fileNameTest As String = $"{testName}.xml"
        ...
        Protected Overridable Sub AddManifestMetaData(request As QTI22PublicationRequest, testPackage As TestPackage, packageType As PackageCreatorConstants.PackageType, manifestName As String, testPackageGuid As String)
        End Sub
