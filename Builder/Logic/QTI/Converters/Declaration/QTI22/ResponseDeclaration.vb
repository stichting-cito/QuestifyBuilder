...
Namespace QTI.Converters.Declaration.QTI22
    ...
    Public Class QTI22ResponseDeclaration
        ...
        Public Function GetDeclarations() As List(Of ResponseDeclarationType)
            ...
            For Each responseIdentifierAttribute As XmlNode In _responseIdentifierAttributeList
                ...
                _finding = CombinedScoringHelper.GetFindingByResponseIdentifier(_solution, responseIdentifierAttribute.Value)
                ...
                If FindingContainsFactsToProcess(_finding) Then
                    ...
                    If facts.Any OrElse responseIsInFactSet Then
                        ...
                        End If
                        AddResponseDeclaration(responseIdentifierAttribute, GetResponseValues(responseIdentifierAttribute, ResponseValueType.CorrectReponse, facts, shouldAddToCorrectResponse), GetResponseValues(responseIdentifierAttribute, ResponseValueType.DefaultValue, facts, shouldAddToCorrectResponse), GetAreaMappings(responseIdentifierAttribute, facts), interPretationValue, list, keyValue)
                    End If
                End If
        ...
        Private Sub AddEmptyResponseDeclaration(ByVal responseIdentifierAttribute As XmlNode, ByRef list As List(Of ResponseDeclarationType))
            ...
            responseDeclarationType.identifier = QTI22ScoringHelper.GetResponseId(GetResponseIndex(responseIdentifierAttribute.Value))
            responseDeclarationType.cardinality = QTI22ScoringHelper.GetResponseDeclarationCardinality(responseIdentifierAttribute)
        ...
        Private Sub AddResponseDeclaration(ByVal responseIdentifierAttribute As XmlNode, ByVal correctResponseValues() As ValueType, ByVal defaultValues() As ValueType, ByVal areaMappings() As AreaMapEntryType, ByVal interPretationValue As String, ByRef list As List(Of ResponseDeclarationType), keyValue As KeyValue)
            Dim responseDeclarationType As New ResponseDeclarationType
            responseDeclarationType.identifier = QTI22ScoringHelper.GetResponseId(GetResponseIndex(responseIdentifierAttribute.Value))
            responseDeclarationType.cardinality = QTI22ScoringHelper.GetResponseDeclarationCardinality(responseIdentifierAttribute)

            If correctResponseValues IsNot Nothing OrElse Not String.IsNullOrEmpty(interPretationValue) Then
                responseDeclarationType.correctResponse = New CorrectResponseType
                If Not String.IsNullOrEmpty(interPretationValue) Then responseDeclarationType.correctResponse.interpretation = interPretationValue
                If correctResponseValues IsNot Nothing Then responseDeclarationType.correctResponse.value = correctResponseValues.ToList()
            End If

            If defaultValues IsNot Nothing Then
                responseDeclarationType.defaultValue = New DefaultValueType
                responseDeclarationType.defaultValue.value = defaultValues.ToList()
            End If

            If areaMappings IsNot Nothing Then
                responseDeclarationType.areaMapping = New AreaMappingType()
                responseDeclarationType.areaMapping.areaMapEntry = areaMappings.ToList()
            End If

            responseDeclarationType.baseType = QTI22ScoringHelper.GetResponseDeclarationBaseType(responseIdentifierAttribute, GetResponseTypeFactory().GetScoringParamFromFactIdsPerScoringParameterById(responseIdentifierAttribute.Value), _finding, keyValue)
            responseDeclarationType.baseTypeSpecified = True
            list.Add(responseDeclarationType)
        End Sub
        ...
        Private Function GetResponseValues(responseIdentifierAttribute As XmlNode, responseValueType As ResponseValueType, facts As List(Of BaseFact), shouldAddToCorrectResponse As Boolean) As ValueType()
            ...
            For x As Integer = 0 To processedFacts.Count - 1
                If processedFacts(x).Values IsNot Nothing AndAlso DirectCast(processedFacts(x).Values(0), KeyValue).Values IsNot Nothing AndAlso Not TypeOf DirectCast(processedFacts(x).Values(0), KeyValue).Values(0) Is NoValue Then
                    ...
                    If responseValueType = QTI22ResponseDeclaration.ResponseValueType.CorrectReponse AndAlso shouldAddToCorrectResponse Then
                        responseValue = GetCorrectResponseValue(CType(processedFacts(x), KeyFact))
                    ElseIf responseValueType = QTI22ResponseDeclaration.ResponseValueType.DefaultValue AndAlso ShouldAddDefaultValue(responseIdentifierAttribute) Then
        ...
        Private Function GetFactsToProcess(responseIdentifierAttribute As XmlNode, facts As List(Of BaseFact)) As List(Of BaseFact)
            If CombinedScoringHelper.IsMultiChoiceScoring(responseIdentifierAttribute, _scoringParams) Then
        ...
        Private Function GetKeyValueForResponse(responseIdentifierAttribute As XmlNode) As KeyValue
            Dim keyvalue As KeyValue = CombinedScoringHelper.GetKeyValueByResponseIdentifier(_finding, responseIdentifierAttribute.Value)
            If keyvalue Is Nothing Then keyvalue = CombinedScoringHelper.GetKeyValueForInputByIndex(_finding, GetIndexForScoringDetermination())
            Return keyvalue
        End Function
        ...
        Private Function GetFactsOnFinding(responseIdentifierAttribute As XmlNode, finding As KeyFinding) As List(Of BaseFact)
            ...
            factsOnFinding = CombinedScoringHelper.GetFactsForInputByIndex(finding, _indexForScoringDetermination)
        ...
        Private Function GetIdentifier(input As String) As String
            ...
            Dim guidPart As String = CombinedScoringHelper.GetGuidPartOfIdentifier(input)
        ...
        Private Sub DetermineIfResponseIsPartOfComposedInteraction(responseIdentifierAttribute As XmlNode, keyvalue As KeyValue)
            ...
            If CombinedScoringHelper.DetermineControlType(DirectCast(responseIdentifierAttribute, XmlAttribute), _scoringParams) = CombinedScoringHelper.EnumControlType.Input Then
                If Not keyvalue Is Nothing AndAlso QTI22ScoringHelper.IsTimeValue(responseIdentifierAttribute, keyvalue) Then
                    _nrOfPartsInComposedInteraction = GapTimeScoringHelper.NrOfTimeParts(keyvalue.Values)
                    Exit Sub
                ElseIf keyvalue IsNot Nothing AndAlso QTI22ScoringHelper.IsDateValue(responseIdentifierAttribute, keyvalue) Then
        ...
        Private Function ShouldAddAreaMapping(responseIdentifierAttribute As Xml.XmlNode) As Boolean
            If DirectCast(responseIdentifierAttribute, XmlAttribute).OwnerElement.Name.ToLower.Equals("selectpointinteraction", StringComparison.InvariantCultureIgnoreCase) Then
                Return True
            End If
            Return False
        End Function
