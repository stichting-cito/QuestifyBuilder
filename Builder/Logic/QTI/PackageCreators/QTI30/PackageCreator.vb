...
Imports Questify.Builder.Logic.QTI.ChainHandlers.Processing.QTI30
Imports Questify.Builder.Logic.QTI.Converters.ScoringConverter.QTI30
Imports Questify.Builder.Logic.QTI.Facade.QTI30
...
Namespace QTI.PackageCreators.QTI30
    ...
    Public Class PackageCreator
        ...
        Protected Function Create(bw As BackgroundWorker, testCollection As ResourceEntryCollection, ByVal newPublicationRequest As PublicationRequest, validate As Boolean, testToPublish As String, isPreview As Boolean) As Boolean
        ...
        Protected Function Create(bw As BackgroundWorker, testPackage As TestPackage, listOfAssessmentTest As List(Of AssessmentTest2), listOfTestRef As List(Of TestReference), ByVal newPublicationRequest As PublicationRequest, validate As Boolean, testToPublish As String, isPreview As Boolean) As Boolean
            If isPreview Then
            ...
            Try
                Using New SessionContextProvider(sessionContext)
                    ...
                    _facade = New PackageCreatorFacade
                    _facade.TestCreationChain.Add(New ProgressHandler(Of PublicationRequest)(DetermineNrOfProgressSteps(listOfAssessmentTest, Not listOfTestRef.Count = 0), AddressOf HandleProgress))
...
#End If
        ...
        Private Sub ProcessTests(testList As List(Of AssessmentTest2), listOfTestRefs As List(Of TestReference))
            ...
            For Each item In _listOfItems.Keys
                _facade.SaveItemAndResourcesChain.Add(New ProgressHandler(Of PublicationRequest)(String.Format(My.Resources.ProcessingItem, item), AddressOf HandleProgress))
        ...
        Protected Overrides Sub NewItemReference(itemReference As ItemReferenceViewBase, testIdentifier As String)
            ...
            End If
            _facade.TestCreationChain.Add(New ProgressHandler(Of PublicationRequest)(String.Format(My.Resources.ProcessingItem, itemReference.Title), AddressOf HandleProgress))
        ...
        Private Sub SetupFacadeForPackaging(ByVal isForPreview As Boolean)
            With _facade
                ...
                .PackagingChain.Add(New ProgressHandler(Of PublicationRequest)(My.Resources.PublishingPackageToSpecifiedLocation, AddressOf HandleProgress))
                ...
                If Not isForPreview AndAlso Not String.IsNullOrEmpty(EncryptionHandlerTypeName) Then
                    Dim t = Type.GetType(EncryptionHandlerTypeName, True)
                    Dim handler = DirectCast(Activator.CreateInstance(t), IChainhandler(Of PublicationRequest))
                    ...
                    If handler IsNot Nothing Then
                        .PackagingChain.Add(New ProgressHandler(Of PublicationRequest)(0, AddressOf HandleProgress))
                        .PackagingChain.Add(New ProgressHandler(Of PublicationRequest)(My.Resources.BusyEncryptingDocument, AddressOf HandleProgress))
        ...
        Protected Overridable Sub SetupFacadeForXsdValidationChain()
            With _facade
                ...
                .SetupXsdValidationChain.Add(New SetupXsdHandler(Me))
            End With
        End Sub
        ...
        Public Overridable Sub SaveGeneratedCss(resources As ConcurrentDictionary(Of String, Dictionary(Of ResourceType, List(Of String))), css As String, fileName As String, cssDirectoryName As String, cssResourceType As ResourceTypeType)
        ...
        Public Overridable Sub SaveGeneratedCss(resources As ConcurrentDictionary(Of String, Dictionary(Of ResourceType, List(Of String))), css As String, cssDirectoryName As String, cssResourceType As ResourceTypeType)
        ...
        Public Function GetScoreConverterFromScoringParameters(ByVal item As AssessmentItem) As IScoringConverter
        ...
        Protected Overridable Function GetAssessmentInitialiseTestHandler(test As AssessmentTest2, qtiTest As GeneralAssessmentTest) As AssessmentInitialiseTestHandler
            Return New AssessmentInitialiseTestHandler(test, qtiTest, Me)
        End Function
        ...
        Public Overridable Function GetCombinedScoreConverter(itemIdentifier As String, scoringParameters As HashSet(Of ScoringParameter)) As IScoringConverter
            Return DirectCast(New CombinedScoringConverter(scoringParameters), IScoringConverter)
        End Function
        ...
        Protected Overridable Function GetAssessmentItemHandler(qtiitem As GeneralItemReference, testIdentifier As String) As AssessmentItemHandler
            Return New AssessmentItemHandler(qtiitem, testIdentifier, Me)
        End Function

        Protected Overridable Function GetCreateAssessmentItemAndResourcesHandler(itemCode As String) As CreateAssessmentItemAndResourcesHandler
            Return New CreateAssessmentItemAndResourcesHandler(itemCode, ListOfItems(itemCode).[Select](Function(i) TestIdentifierToCompare(i.Key)).ToList(), Me)
        End Function
        ...
        Protected Overridable Function GetAssessmentTestPartHandler(qtiTestPart As GeneralTestPart, testname As String) As AssessmentTestPartHandler
            Return New AssessmentTestPartHandler(qtiTestPart, testname, Me)
        End Function

        Protected Overridable Function GetPackageZipFileCreatorHandler() As PackageZipFileCreatorHandler
            Return New PackageZipFileCreatorHandler(Me)
        End Function

        Protected Overridable Function GetSaveTestAndManifestHandler(testPackage As TestPackage, testPackageGuid As String) As SaveTestAndManifestHandler
            Return New SaveTestAndManifestHandler(testPackage, testPackageGuid, Me)
        End Function

        Protected Overridable Function GetTestReferenceHandler(testRef As TestReference) As TestReferenceHandler
            Return New TestReferenceHandler(testRef)
        End Function

        Protected Overridable Function GetPublicationRequest(sourcePackage As FileInfo, targetPackageFileSystemInfo As FileInfo) As PublicationRequest
            Return New PublicationRequest(sourcePackage, targetPackageFileSystemInfo, AddressOf ValidationEventHandler)
        End Function

        Public Overridable Function GetXsdFolders(request As PublicationRequest) As Dictionary(Of String, String)
        ...
        Public Shared Function ObjectToAny(objectToConvert As Object, ns As Serialization.XmlSerializerNamespaces) As XmlElement
            Return ChainHandlerHelper.ObjectToAny(objectToConvert, ns)
        End Function
