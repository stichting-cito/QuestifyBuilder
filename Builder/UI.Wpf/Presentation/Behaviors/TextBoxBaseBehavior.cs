...
{
    internal abstract class TextBoxBaseBehavior<TInputValidator> : Behavior<TextBox>
    where TInputValidator : IInputValidationStrategy, new()
    {
        ...
        {
            var source = (TextBox)e.Source;
            if (!InputValidationStrategy.IsInputValid(source.Text))
                source.Text = _original;
        }
