...
namespace Questify.Builder.UnitTests.Questify.Builder.UI.Presentation.ItemEditor.ScoreEditor
{
    ...
    public class ScoreEditorForEncodingViewModelTests : MVVMTestBase
    {
        ...
        public void ValidateAddingResponseCategoryLogicForSingle()
        {
            ...
            single.GetConceptManipulator(_Solution); //this ensures that concept structure is created
            ...
            test.ProcessResult();
            //Assert
            Assert.AreEqual(2 + 1, _Solution.ConceptFindings[0].Facts.Count); //A Catch all has been added
            Assert.IsTrue(_Solution.ConceptFindings[0].Facts[1].Id.StartsWith("5[*]"));
        ...
        public void ValidateAddingResponseCategoryLogicForGroup()
        {
            ...
            test.ProcessResult();
            //Assert
            Assert.AreEqual(5 + 1, _Solution.ConceptFindings[0].KeyFactsets.Count); //5 plus 1 catch all.
            Assert.AreEqual("1[2]-integerScore",_Solution.ConceptFindings[0].KeyFactsets[5].Facts[0].Id);
        ...
        public void FixCrashWith2GroupedMC()
        {
            ...
            test.ProcessResult();
            //Assert
            Assert.AreEqual(3, _Solution.ConceptFindings[0].KeyFactsets.Count); //1 copied answer,.. 1 catch all, 1 anser category BA
            Assert.AreEqual(2, _Solution.ConceptFindings[0].KeyFactsets[2].Facts.Count);
        ...
        public void AssertThatFactIdIsCreatedAsIntended()
        {
            ...
            test.ProcessResult();
            //Assert
            Assert.AreEqual(3, _Solution.ConceptFindings[0].Facts.Count); //1 copied answer,.. 1 catch all, 1 answer
        ...
        public void AssertThatFactIdIsCreatedAsIntended_2()
        {
            ...
            //--2
            single.GetConceptManipulator(_Solution); //this ensures that concept structure is created
            test = new ScoreEditorForEncodingViewModel(single, _Solution);
            manipulator = scoringPrms[0].GetScoreManipulator(test.WorkingSolution);
            manipulator.SetKey("I23a0653b-b574-4d5e-ad66-e05af1a169d", "2");
            test.ProcessResult();
            
            //Assert
            Assert.AreEqual(4, _Solution.ConceptFindings[0].Facts.Count); //1 copied answer,.. 1 catch all, 1 answer

            Assert.AreEqual("I23a0653b-b574-4d5e-ad66-e05af1a169d[1]", _Solution.ConceptFindings[0].Facts[2].Id);
            Assert.AreEqual("I23a0653b-b574-4d5e-ad66-e05af1a169d[2]", _Solution.ConceptFindings[0].Facts[3].Id);
        }

        #region Solution handeling
