...
Public Class GenericResourceEditorBehaviorTests
    ...
    Public Sub InlineCount_HtmlConverterTest()
        ...
        Assert.AreEqual(1, behavior.InlineElements.Where(Function(ie) ie.Value.Item2 = True).Count())  'Old model inline element, indicating that inline images should be converted to img tag instead of cito inline control
        Assert.AreEqual(0, behavior.InlineElements.Where(Function(ie) ie.Value.Item2 = False).Count()) 'New inline elements, that should be converted to cito inline controls
    End Sub
    ...
    Public Sub InlineBack2Parameter_HtmlConverterTest()
        ...
        Assert.IsTrue(tmp.Contains("<img")) 'Does not contain old image
        Assert.IsFalse(tmp.Contains("<cito:InlineElement")) 'Does not contain cito inline element
