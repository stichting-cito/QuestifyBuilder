...
Public Class TestSetPropertyEditorContainer

    Public Sub InitializePropertyEditorsAndSetDataSource(ByVal testSet As TestSet, ByVal viewTypes As List(Of String))
        Me.SuspendLayout()

        For Each propertyEditorBaseRef As TestPackageEditorControlBase In Me.CurrentPropertyEditorControls
            Dim currentPropertyEditor As ITestSetEditorPropertyEditor = DirectCast(propertyEditorBaseRef, ITestSetEditorPropertyEditor)
        ...
        Dim genericPropertyEditor As New General_TestSetPropertiesEditor()
        InitializePropertyEditor(genericPropertyEditor, testSet)

        For Each viewType In viewTypes
            Dim editor As ITestSetEditorPropertyEditor = GetPropertyEditorBasedInViewType(viewType)
            InitializePropertyEditor(editor, testSet)
        Next

        Me.ResumeLayout()
    End Sub

    Private Function GetPropertyEditorBasedInViewType(ByVal viewType As String) As ITestSetEditorPropertyEditor
        ...
        Return plugin.GetTestSetPropertiesEditor()
    End Function

    Private Sub InitializePropertyEditor(ByVal editor As ITestSetEditorPropertyEditor, ByVal dataSource As TestSet)
